...
func (c *Client) GetSearchResults(ctx context.Context, req *SearchRequest) ([]entities.SearchResult, error) {
	...
	results2 := make([]entities.SearchResult, len(results1))
	for i := range results1 {
		results2[i] = entities.SearchResult{
	...
	//
	// The `GetSearchResults` method adapts the `resp` object returned from `c.client.GetSearchResults` method to a new object of type `[]entities.SearchResult`.
	// It achieves this by mapping the properties of the objects returned by the external service to a new object of the required type `entities.SearchResult`.
	//
	...
	// not directly compatible with the `entities.SearchResult` type used in the client code, and the adapter method maps the response to the desired type.
}
